...
BULLET_FIRE_SOUND = pygame.mixer.Sound(os.path.join('Assets', 'gunshoot.wav'))
SOMEONE_WINS_SOUND = pygame.mixer.Sound(os.path.join('Assets', 'you-win.wav'))
    ...
    while run:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_LCTRL and len(yellow_bullets) < MAXIMUM_BULLETS:
                    bullet = pygame.Rect(yellow.x + yellow.width, yellow.y + yellow.height//2 - 2, 10, 5)
                    yellow_bullets.append(bullet)
                    BULLET_FIRE_SOUND.play()

            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_RCTRL and len(red_bullets) < MAXIMUM_BULLETS:
